...
import { applyOffers,getActiveOffers } from "../utils/offerUtils.js";
...
export const getDetailPage = async (req, res) => {
  try {
   ...
   const activeOffers = await getActiveOffers();
    ...
    const gameWithOffers = applyOffers(game,activeOffers)
    ...
    const relatedCompanyWithOffers = relatedCompanies.map(game=>
        applyOffers(game,activeOffers)
    )
    ...
    const relatedGamesWithOffers = relatedGames.map(game=>
        applyOffers(game,activeOffers)
    )
    ...
    res.render("user/gamedetail", {
      game:gameWithOffers,
      relatedGames:relatedGamesWithOffers,
      availableStock,
      isWishlisted,
      relatedCompanies:relatedCompanyWithOffers,
      user,
      page: "gameDetail",
      cartCount,
    });
